...
function buildPayload(form: SessionForm) {
  return {
...
export default function AssignedSessionDetailPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isNew) {
        const createdId = asString((data as Record<string, unknown>).sessionId);
        if (createdId) {
          router.replace(`/coach/assigned-sessions/${createdId}`);
          return;
        }
      }
      await loadDetail();
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ display: "grid", gap: 6 }}>
                        ...
                        >
                          {selected?.name || row.exerciseSearch || "Nessun esercizio selezionato"}
                        </div>
                      </div>
                      <p style={{ fontSize: 12, opacity: 0.7 }}>
                        Per RUN/ERG usa testo in reps o note (es. "30' Z2").
                      </p>
                      ...
                      >
                        ...
                        <label style={{ display: "grid", gap: 6 }}>
                          ...
                          <input
                            value={row.percent}
                            onChange={(e) =>
                              updateRow(blockIndex, rowIndex, { percent: e.target.value })
                            }
                          />
                        </label>
                        <label style={{ display: "grid", gap: 6 }}>
