...
export async function PATCH(
  ...
  const existing = await prisma.competition.findUnique({
    where: { competitionId },
    select: { competitionId: true, coachId: true, dateStart: true },
  });
  ...
  if (existing.coachId !== session.uid) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  ...
  if (body && typeof body === "object") {
    ...
    if ("startDate" in body) {
      ...
      if (!raw) return badRequest("startDate non valida");
      const parsed = parseDateInput(raw);
      if (!parsed) return badRequest("startDate non valida");
    ...
    if ("endDate" in body) {
      ...
      if (!raw) {
        ...
        const parsed = parseDateInput(raw);
        if (!parsed) return badRequest("endDate non valida");
        data.dateEnd = parsed;
      }
    }
...
export async function DELETE(
  ...
  await prisma.$transaction(async (tx) => {
    await tx.competitionAssignment.deleteMany({
      where: { competitionId },
    });
    await tx.competition.delete({ where: { competitionId } });
  });
